...
export default (
  ...
  const getBoundsContainer = () => editor.queryCommandValue('ToggleView') === '' ? editor.getContentAreaContainer() : editor.getContainer();

  const getBounds = () => {
    const contentArea = Boxes.box(SugarElement.fromDom(getBoundsContainer()));
            ...
            ...(
              Options.isStickyToolbar(editor) && !sharedBackstage.header.isPositionedAtTop()
                ...
                : [
                  Docking.config({
                    contextual: {
                      lazyContext: () => Optional.some(Boxes.box(SugarElement.fromDom(getBoundsContainer()))),
